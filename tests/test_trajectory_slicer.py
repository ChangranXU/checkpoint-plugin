...
def test_session_boundary_recovers_trailing_task_complete(tmp_path):
    ...
    body = (
        _codex_meta("agent") + _codex_turn("t1") + _codex_turn("t2") + _codex_turn("t3")
    )
    ...
    ref = TrajectoryReference(
        "codex",
        str(rollout),
        len(_codex_meta("agent").encode()),
        end,
        3,
        boundary_mode="session_boundary",
    )
...
def test_session_boundary_stops_at_new_session_meta(tmp_path):
    ...
    ref = TrajectoryReference(
        "codex", str(rollout), 0, end, 2, boundary_mode="session_boundary"
    )
...
def test_recover_tail_missing_or_truncated_file_returns_empty(tmp_path):
    ...
    shrunk = TrajectoryReference(
        "codex", str(rollout), 0, 10_000, 1, boundary_mode="session_boundary"
    )
...
def test_jsonl_after_leading_metas_tags_session_boundary(tmp_path):
    ...
    ref = jsonl_after_leading_metas(
        "codex", rollout, is_leading_meta=lambda r: r.get("type") == "session_meta"
    )
...
def test_trajectory_reference_boundary_mode_roundtrips():
    ...
    ref = TrajectoryReference(
        "codex", "/x.jsonl", 5, 99, 3, boundary_mode="session_boundary"
    )
    assert ref.to_json()["boundary_mode"] == "session_boundary"
    assert (
        TrajectoryReference.from_json(ref.to_json()).boundary_mode == "session_boundary"
    )
